              ...
              itemBuilder: (context, index) {
                return Card(
                  ...
                  child: Column(
                    ...
                    children: [
                      buildListTile("User Email", state.requests?[index].email??""),
                      buildListTile("Budget Name", state.requests?[index].name ?? ''),
                      buildListTile(
                          "Amount", "${state.requests?[index].amount ?? 0}"),
                      buildListTile("Budget Type", state.requests?[index].type ?? ''),
                      buildListTile("Status", state.requests?[index].status ?? ''),
                      buildListTile("Start Date", state.requests?[index].date ?? ''),
                      buildListTile("End Date", state.requests?[index].expected_date ?? ''),
                      SizedBox(height: 10),
                      buildButtons(context, state.requests?[index].docId ?? "", state.requests?[index].userId ?? ""),
                    ],
                  ),
                );
              },
